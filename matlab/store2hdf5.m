function [curr_dat_sz, curr_lab_sz, curr_led_sz, curr_hed_sz] = store2hdf5(filename, data, labels, ledge, hedge, create, startloc, chunksz)  
  ...
  if create
    ...
    h5create(filename, '/ledge', [led_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [led_dims(1:end-1) chunksz]); % width, height, channels, number 
    h5create(filename, '/hedge', [hed_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [hed_dims(1:end-1) chunksz]); % width, height, channels, number 
    if ~exist('startloc','var') 
      startloc.dat=[ones(1,length(dat_dims)-1), 1];
      startloc.lab=[ones(1,length(lab_dims)-1), 1];
      startloc.led=[ones(1,length(led_dims)-1), 1];
      startloc.hed=[ones(1,length(hed_dims)-1), 1];
    end 
  else  % append mode
    if ~exist('startloc','var')
      ...
      prev_led_sz=info.Datasets(3).Dataspace.Size;
      prev_hed_sz=info.Datasets(4).Dataspace.Size;
      ...
      assert(prev_led_sz(1:end-1)==led_dims(1:end-1), 'Ledge dimensions must match existing dimensions in dataset');
      assert(prev_hed_sz(1:end-1)==hed_dims(1:end-1), 'Hedge dimensions must match existing dimensions in dataset');
      startloc.dat=[ones(1,length(dat_dims)-1), prev_dat_sz(end)+1];
      startloc.lab=[ones(1,length(lab_dims)-1), prev_lab_sz(end)+1];
      startloc.led=[ones(1,length(led_dims)-1), prev_led_sz(end)+1];
      startloc.hed=[ones(1,length(hed_dims)-1), prev_hed_sz(end)+1];
    end
  end
  ...
  if ~isempty(data)
    ...
    h5write(filename, '/ledge', single(ledge), startloc.lab, size(ledge)); 
    h5write(filename, '/hedge', single(hedge), startloc.lab, size(hedge)); 
  end
  ...
  if nargout
    ...
    curr_led_sz=info.Datasets(3).Dataspace.Size;
    curr_hed_sz=info.Datasets(4).Dataspace.Size;
  end
end
